  ...
  describe('When logged in', function() {
    ...
    describe('and several blogs exist', function() {
      beforeEach(function () {
        ...
        cy.contains('another blog cypress').parent().find('button').as('expandButton')
        cy.get('@expandButton').click()

        cy.contains('one more blog cypress').parent().find('button').as('expandButton')
        cy.get('@expandButton').click()
      })

      // 5.20
      it('a blog can be liked', function() {
        cy.contains('another blog cypress').parent().parent()
          .contains('0 likes')

        cy.contains('another blog cypress').parent().parent().find('.like-btn').as('likeButton')
      ...
      it('own blog can be removed', function() {
        cy.contains('remove')
        cy.contains('one more blog cypress').parent().parent().find('.remove-btn').as('removeButton')
      ...
      it('blogs are arranged by number of likes in a descending order', function() {
        cy.get('.blog-container').eq(0).should('contain', 'another blog cypress')
        cy.get('.blog-container').eq(1).should('contain', 'one more blog cypress')

        cy.contains('one more blog cypress').parent().parent().find('.like-btn').as('likeButton')
        cy.get('@likeButton').click()

        cy.get('.blog-container').eq(0).should('contain', 'one more blog cypress')
        cy.get('.blog-container').eq(1).should('contain', 'another blog cypress')
